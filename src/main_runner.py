...
if __name__ == '__main__':
    ...
    if generate_synthetic_data:
        ...
        if params.type_of_input['tag'] == 'poly':
            ...
            a, b, c, d = params.type_of_input['a'], params.type_of_input['b'], params.type_of_input['c'], \
...
if __name__ == '__main__':
    ...
    if gen_adversarial:
        ...
        if tactic[0] == 'std_ratio':
            ...
            weight_sum = sum(info['DISTANCES'])
            for idx, row in centroids.iterrows():
                drow = row[data_cols]
                dweight = info['DISTANCES'][idx]
            ...
            for adversary in range(num_of_adversaries):
                ...
                while True:
                    ...
                    for p in data_cols:
                        is_good = is_good \
                                  and ((True and adversary_pos_center['{}_min'.format(p)] < adv_pos[p]) and adv_pos[p] <
...
if __name__ == '__main__':
    ...
    if export_raw_data:
        df_xport = df.append(adv_pos, ignore_index=True)
        outpath = os.path.join(paths.result_path, 'data_with_adv_BASE.png')
...
if __name__ == '__main__':
    ...
    if open_pool_if_needed:
        info = params.attack_tactic[1]
        if info.get('workload_handling', 'NULL') == 'parallel':
                ...
                if attack_data:
                    ...
                    if attack_sig == 'by_distance_from_adv':
                        ...
                        if compute_distances:
                            ...
                            attack_df['dist_from_adv'] = df.apply(lambda row: get_dist(row, adv_pos, data_cols), axis=1)
                        ...
                        if remove_from_blue:
                            blue_budget = int(budget * info['budget_for_remove_blue'])
                        ...
                        if remove_from_red:
                            red_budget = int(budget * info['budget_for_remove_red'])
                    ...
                    if attack_sig == 'greedy_search':
                        k = budget
                        workload = info['workload_handling']
                            ...
                            for gen in range(generations):
                                ...
                                if create_new_gen:
                                    ...
                                    for offspring_idx in range(parents, offsprings):
                                        ...
                                        if mutate_offspring:
                                            offspring['REPLACE'] = [
                                                np.random.choice([True, False], p=[mutation_rate, 1 - mutation_rate])
                                                for _ in range(k)]
                    ...
                    del attack_data, attack_sig, info
