...
class DataOmittor:
    ...
    def make_train_set(self, train_idx):
        ...
        clear_folder(self.attacked_train_dir, clear_if_exist=True)
        for c_class in self.attacked_train['label'].unique():
            ppath = os.path.join(self.attacked_train_dir, c_class)
            clear_folder(ppath, clear_if_exist=True)

        clear_folder(self.ommited_dir, clear_if_exist=True)
        for c_class in self.omitted_train['label'].unique():
...
def experiment_instance(randomseed=0, thread_name='', budget=25):
    import sys
    ...
    predicted_class_before_attack = 1.0
    ...
    msg += f'learning type: {"TRANSFER"}' + '\n'
    msg += f'Debug mode: {DEBUG_MODE}' + '\n'
    ...
    report_path = os.path.join(work_dir,
                               f'Report_{selected_random_seed:>04}_{learner_type}_TRANSFER_Budget{int(budget)}_{symbol}.txt')
...
def random_budget(idx):
    d = {0: 25, 1: 50, 2: 100, 3: 5}
    return d[idx % (len(d))]
