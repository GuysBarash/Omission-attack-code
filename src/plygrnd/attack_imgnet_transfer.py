...
class KNNDetector:
    ...
    def get_vectors(self, paths, batch_size=20):
        ...
        for img_path in tqdm(paths, total=len(paths), desc='KNNDetecor opening imgs'):
            with Image.open(img_path).convert('RGB') as o_img:
                to_img = self.transform(o_img)
                imgs_t.append(to_img)
...
class DataOmittor:
    ...
    def get_train_size(self):
        return len(self.main_dataset.samples)
...
def experiment_instance(randomseed=0, thread_name='', budget=500):
    ...
    print(f'Original samples count: {omittor.get_train_size()}')

    info_db['SRC_CLASS'] = omittor.src_class
    info_db['TRGT_CLASS'] = omittor.trgt_class
    info_db['train_samples_count_original'] = omittor.get_train_size()
    ...
    img = Image.open(omittor.get_adv_path(exact=True)).convert('RGB')
    img_size = 224, 224
    ...
    removal_method = 'from_all_classes'
    idx_to_remove = list()
    ...
    attacked_db_labels_count = idx_to_keep_df.groupby('label')['class'].count()
    display(attacked_db_labels_count)

    print(f"Removing: [{idx_to_remove_df.shape[0]}]")
    # display(HTML(idx_to_remove_df.to_html()))
    # print(tabulate(idx_to_remove_df.head(40), headers='keys', tablefmt='psql'))
    print(f"Remain: [{idx_to_keep.shape[0]}]")
    # display(HTML(idx_to_remove_df.to_html()))
    # print(tabulate(idx_to_keep_df.head(40), headers='keys', tablefmt='psql'))
    info_db['train_samples_count_final'] = idx_to_keep.shape[0]
    ...
    learner = Learner(transform, num_classes=omittor.get_labels_count(),
                      model_type=learner_type, src_clas=omittor.src_class, trgt_class=omittor.trgt_class)
    ...
    for t_class, t_count in attacked_db_labels_count.items():
        msg += f"samples {t_class}: {int(t_count)}\n"
